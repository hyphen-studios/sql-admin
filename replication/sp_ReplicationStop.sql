CREATE PROCEDURE [dbo].[sp_ReplicationStop] @Publication NVARCHAR(75), @Subscriber NVARCHAR(75), @DestDBName NVARCHAR(150)
AS
/******************************************************************************
...
SET @TSQL = N'USE ['+@PubDBName+']; exec sp_dropsubscription @publication = N'''+@Publication+''', @subscriber = N'''+@Subscriber+''', @destination_db = N'''+@DestDBName+''', @article = N''all'''
--PRINT @TSQL
...
SET @TSQL = N'exec sp_subscription_cleanup @publisher = N'''++@Subscriber++''', @publisher_db = N'''+@PubDBName+''', @publication = N'''+@Publication+''''

-- Dropping the transactional publication
SET @TSQL = N'use  ['+@PubDBName+']; exec sp_droppublication @publication = N'''+@Publication+''''
EXEC sp_executesql @TSQL

-- Disabling the replication database
SET @TSQL = N'USE master; exec sp_replicationdboption @dbname = N'''+@PubDBName+''', @optname = N''publish'', @value = N''false'''
EXEC sp_executesql @TSQL

SET @TSQL = N'USE master; EXEC sp_removedbreplication N'''+@DestDBName+''''
EXEC sp_executesql @TSQL
